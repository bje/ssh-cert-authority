...
ssh-cert-authority:
	go build -ldflags "-X ${PKG}/version.Tag=${TAG} -X ${PKG}/version.BuildVersion=${VERSION}" -v ./cmd/ssh-cert-authority

ssh-cert-authority-linux-amd64:
	GOOS=linux GOARCH=amd64 \
		 go build -o ssh-cert-authority-linux-amd64 \
		 	-ldflags "-X ${PKG}/version.Tag=${TAG} -X ${PKG}/version.BuildVersion=${VERSION}" -v ./cmd/ssh-cert-authority
